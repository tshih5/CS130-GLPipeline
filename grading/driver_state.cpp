...
void render(driver_state& state, render_type type)
{
    ...
    switch(type){
		...
		case render_type::triangle:{
			...
			for(int i = 0; i <= state.num_vertices / 3; ++i){							//for each triangle 
		...
		case render_type::fan:{
			...
			for(int i = 0; i < num_triangles; ++i){										//for each triangle 
				for(int j = 0; j < 3; ++j){												//for each vertex of the triangle,
					if(i != 0 && j == 0){												//after the first triangle, we do not recalculate the first vertex;
						continue;
					}
					vertex.data = &state.vertex_data[vertex_ind];						//point to the address of the start of the vertex
					temp_triangle[j].data = vertex.data;								//add vertex to temp triangle vertex j
